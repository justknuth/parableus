...
// import { doc, collection, query, orderBy, onSnapshot, serverTimestamp, writeBatch, increment } from 'firebase/firestore';
...
//     // Real-time listener for the theory
...
//     // Real-time listener to check if the current user has upvoted
...
//   useEffect(() => {
//     // Initialize fork inputs exactly once after the theory loads
//     if (theory && !forkTitle) {
...
//                 disabled={!user}
//                 className={`p-2 rounded-full transition-colors disabled:opacity-50 disabled:cursor-not-allowed ${hasUpvoted ? 'bg-[#3c2f2f] text-[#e8dcc7]' : 'text-[#a89b8f] hover:bg-[#2a2422] hover:text-[#e8dcc7]'}`}
//               >
//                 <ArrowUp size={28} />
//               </button>
        ...
        >
          <div className="flex items-start space-x-6">
            <div className="flex flex-col items-center space-y-2">
              <button 
                onClick={handleUpvote}
                className={`p-2 rounded-full transition-colors ${hasUpvoted ? 'bg-[#3c2f2f] text-[#e8dcc7]' : 'text-[#a89b8f] hover:bg-[#2a2422] hover:text-[#e8dcc7]'}`}
              >
                <ArrowUp size={28} />
              </button>
              <span className="text-xl font-medium text-[#e8dcc7]">{theory.upvotes}</span>
