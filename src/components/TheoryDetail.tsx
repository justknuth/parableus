...
import { doc, getDoc, collection, query, orderBy, onSnapshot, addDoc, serverTimestamp, updateDoc, increment, writeBatch } from 'firebase/firestore';
...
export default function TheoryDetail({ user }: { user: User | null }) {
  ...
  useEffect(() => {
    ...
    const fetchTheory = async () => {
      try {
        ...
        if (docSnap.exists()) {
          ...
          setTheory(data);
          setForkTitle(`Evolution of: ${data.title}`);
          setForkContent(data.content);
        } else {
    ...
    const q = query(
      collection(db, `theories/${theoryId}/comments`),
  ...
  const handleUpvote = async () => {
    if (!user || !theory) return;
    
    // Optimistic UI update
    setTheory(prev => prev ? { ...prev, upvotes: prev.upvotes + 1 } : null);

    try {
      const response = await fetch(`/api/theories/${theory.id}/upvote`, {
        method: 'POST',
        headers: {
  ...
  const handleComment = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setTheory(prev => prev ? { ...prev, commentCount: prev.commentCount + 1 } : null);
    } catch (err) {
  ...
  const handleFork = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error('Failed to fork theory');
      }
      ...
      <div className="max-w-3xl mx-auto p-6 md:p-12">
              ...
              >
                ...
                <form onSubmit={handleFork} className="space-y-4">
                  ...
                  <div className="flex justify-end space-x-3 pt-4">
                    ...
                    <button
                      type="submit"
                      disabled={isSubmittingFork || !forkTitle.trim() || !forkContent.trim()}
                      className="px-6 py-2 bg-[#3c2f2f] hover:bg-[#4a3b3b] text-[#e8dcc7] rounded-xl transition-colors disabled:opacity-50 flex items-center space-x-2"
                    >
                      {isSubmittingFork ? (
                        <span>Evolving...</span>
                      ) : (
        ...
        >
          <div className="flex items-start space-x-6">
            <div className="flex flex-col items-center space-y-2 text-[#a89b8f]">
              <button 
                onClick={handleUpvote}
                disabled={!user}
                className="p-2 rounded-full hover:bg-[#2a2422] hover:text-[#e8dcc7] transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
              >
                <ArrowUp size={28} />
              </button>
              <span className="text-xl font-medium text-[#e8dcc7]">{theory.upvotes}</span>
            </div>
        ...
        <section>
          ...
          {user ? (
            ...
            <div className="bg-[#1a1614] border border-[#2a2422] rounded-2xl p-6 text-center text-[#a89b8f] mb-12">
