...
export default function Chat({ user }: { user: User | null }) {
  ...
  //         temperature: 1.2, // Higher temperature for more creative/hallucinatory responses
  //       }
  //     });

  //     // Send previous history (excluding the first welcome message to save tokens if needed, but let's just send all)
  ...
  //       role: 'model',
  //       content: response.text || "The void remains silent..."
  //     };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // 2. Format the sliced history for the SDK
